...
| Primary action | Resin shell, Haze reading fill **in the palette's own colour** with the ink tested against it, optical pressure and light; hover/pressed/focus/disabled | `.cr-button` with no modifier; `--cr-haze-own-fill` and `--cr-content-own-text`. `.secondary`, `.quiet` and `.danger` keep the neutral reading fill, which is what makes the tinted one read as primary. Real action and progress/error handling are app-owned |
...
Buttons, action links, tabs, selectable controls and field shells use Resin as their interaction surface, with Haze reading protection inside a visible Resin perimeter. **On the primary action that reading fill carries the palette's colour rather than the perimeter doing it** — `--cr-haze-own-fill` with `--cr-content-own-text`, the same pair an authored message bubble uses. The soft tone, not the solid one: a Haze fill is 80% opaque and transmits whatever is behind it, so the full primary composited over Crystal's own canvas and surface lands between 3.37 and 4.55 against `onPrimary` in every light palette, below the 4.5 floor in all six. The exported pair is 8.40 to 12.34 in both modes, and its ink is per-palette, which is why Harbor takes its ordinary text ink where the others take `onPrimarySoft`. The recipe is in `assets/crystal.css`, in `@layer crystal.component`: a Resin fill with a rim and a float shadow, a `::before` carrying the inset Haze fill at the material's feather, and a `::after` carrying the optical sheen from `--cr-control-color` and `--cr-control-light`. Both pseudo-elements are dropped under reduced transparency and under forced colours.
