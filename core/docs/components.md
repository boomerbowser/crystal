...
| Primary action | Resin shell, Haze reading fill **in the primary colour** with the tested ink on it, optical pressure and light; hover/pressed/focus/disabled | `.cr-button` with no modifier; `--cr-primary` on the fill and `--cr-on-primary` as the ink. `.secondary` and `.danger` keep the neutral reading fill; `.quiet` has no reading fill at all, so its label sits directly on the material. Primary is tinted, secondary is the neutral pad, quiet is neither. Real action and progress/error handling are app-owned |
