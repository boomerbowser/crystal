...
| Token | Type | Value | Meaning |
| --- | --- | --- | --- |
| `component.progress.ringStroke` | dimension | `8px` | Stroke of a circular progress ring, and of the gauge arc that the catalogue says matches it. One value so the two cannot drift: a gauge is a progress ring that has been told what it is measuring. |
