...
shape:{title:'A recognizable shape language',body:'Ordinary content uses a consistent soft radius. Authored or directional content can use one tighter corner, inherited from Gather’s asymmetric messages. Keep that corner meaningful and mirror it for right-to-left interfaces.',classes:'.cr-bubble / .cr-bubble.own'},
...
for(const [id,p] of Object.entries(CRYSTAL_TOKENS.palettes)){
  const swatch=document.createElement('button');swatch.type='button';swatch.className='swatch';swatch.style.setProperty('--swatch',p.seed);swatch.dataset.palette=id;swatch.setAttribute('aria-label',p.name+' palette');swatch.title=p.name;swatch.setAttribute('aria-pressed','false');$('#palette-picker').append(swatch);
  const card=document.createElement('button');card.type='button';card.className='palette-card';card.dataset.palette=id;card.setAttribute('aria-label','Use '+p.name+' palette');card.setAttribute('aria-pressed','false');card.style.setProperty('--seed',p.seed);card.style.setProperty('--pair',p.companion);const art=document.createElement('div');art.className='palette-art';art.setAttribute('aria-hidden','true');const name=document.createElement('strong');name.textContent=p.name;const hex=document.createElement('small');hex.textContent=p.seed;card.append(art,name,hex);$('#theme-overview').append(card);
}
